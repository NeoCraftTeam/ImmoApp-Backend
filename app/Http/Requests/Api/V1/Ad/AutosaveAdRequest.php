...
/**
 * Validation rules for the autosave endpoint. Every field is `sometimes`
 * so a partial payload can be flushed on a debounce without tripping
 * required rules; `attributes.*` mirrors `SaveAdEditDraftRequest`
 * (exists + active) so an invalid slug can't be stashed on autosave.
 */
final class AutosaveAdRequest extends FormRequest
{
