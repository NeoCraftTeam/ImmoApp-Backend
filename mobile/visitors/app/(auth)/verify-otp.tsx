...
export default function VerifyOtpScreen() {
  ...
  // L'inscription arrive ici via `router.replace` (pas d'historique) —
  // un `back()` nu ne ferait rien. Fallback explicite vers l'inscription
  // pour permettre de corriger une faute de frappe dans l'email.
  ...
  // Deux cas pour « modifier l'email » : compte déjà créé et token en
  // session (inscription ou onglet compte) → correction inline via
  ...
  const handleSaveEmail = async () => {
    ...
    setSavingEmail(true);
    try {
      // Endpoint dédié sous auth/ (seul préfixe accessible à un compte
      // non vérifié) : corrige l'adresse ET renvoie l'OTP.
