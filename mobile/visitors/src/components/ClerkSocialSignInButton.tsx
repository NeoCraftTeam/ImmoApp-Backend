...
import { extractAuthErrorMessage, setBearerToken } from '@/api/client';
import {
...
/**
 * Social sign-in via Clerk SSO (same stack as keyhome.app web).
 * Must render inside {@link OptionalClerkProvider}.
 */
export function ClerkSocialSignInButton({
  ...
  if (iconOnly) {
    return (
      <XStack
        ...
        hitSlop={6}
      >
        {isBusy ? <Spinner /> : <SvgXml xml={iconXml} width={24} height={24} />}
      </XStack>
    );
  }
  ...
  return (
    <XStack
      ...
      accessibilityState={{ disabled: Boolean(disabled), busy: isBusy }}
    >
      {isBusy ? <Spinner /> : <SvgXml xml={iconXml} width={20} height={20} />}
...
async function runClerkSocialSignIn(options: {
  ...
  // Session Clerk résiduelle (Google interrompu, OTP non validé, etc.) bloque startSSOFlow.
  if (isSignedIn) {
    await clerkSignOut?.();
  }
