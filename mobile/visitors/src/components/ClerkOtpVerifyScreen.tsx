...
/** OTP après inscription Google via Clerk — nécessite {@link OptionalClerkProvider}. */
export function ClerkOtpVerifyScreen({ emailHint }: Props) {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (!clerkToken) {
        Alert.alert('Session expirée', 'Relancez la connexion Google.');
  ...
  const handleResend = async () => {
    ...
    try {
      const clerkToken = await getToken();
      if (!clerkToken) {
        Alert.alert('Session expirée', 'Relancez la connexion Google depuis l’écran de connexion.');
        router.replace('/(auth)/login' as never);
        return;
      }
      ...
      if (result.kind === 'otp_required') {
        ...
        Alert.alert(
          'Code renvoyé',
          'Un nouveau code a été envoyé par KeyHome. Vérifiez votre boîte Gmail (onglets Principal, Promotions et Courrier indésirable).',
        );
        return;
      }
      ...
      >
        ...
        <YStack alignItems="center" gap={8}>
          ...
          <Paragraph fontSize={13.5} color="$slate500" textAlign="center" lineHeight={20}>
            {emailHint
              ? `KeyHome a envoyé un code à 6 chiffres à l’adresse Google ${emailHint}.`
              : 'KeyHome a envoyé un code à 6 chiffres à l’adresse de votre compte Google.'}
          </Paragraph>
          <Paragraph fontSize={12.5} color="$slate500" textAlign="center" lineHeight={18}>
            Ce n’est pas une adresse @keyhome.app — c’est normal avec « Continuer avec Google ».
            Pensez aux spams et à l’onglet Promotions Gmail.
          </Paragraph>
        </YStack>
