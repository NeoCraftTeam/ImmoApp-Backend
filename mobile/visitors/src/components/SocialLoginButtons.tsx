...
interface Props {
  /** Called after a successful social sign-in (navigate to home). */
  onSuccess: () => void;
  disabled?: boolean;
}

/**
 * Trois boutons de connexion sociale (Google / Facebook / GitHub),
 * répliquant l'UX web. Chaque bouton ouvre le flux OAuth via le
 * navigateur système (SessionProvider.signInWithProvider) et navigue
 * en cas de succès. Le logo GitHub s'adapte au thème (blanc en sombre).
 */
export function SocialLoginButtons({ onSuccess, disabled }: Props) {
  ...
  const providers: { key: SocialProvider; label: string; xml: string }[] = [
    { key: 'google', label: 'Continuer avec Google', xml: GOOGLE_SVG },
    { key: 'facebook', label: 'Continuer avec Facebook', xml: FACEBOOK_SVG },
    { key: 'github', label: 'Continuer avec GitHub', xml: githubSvg(colors.mutedIcon) },
  ];
