...
class OAuthService {
    ...
    async authenticateWithBackend(
        ...
        try {
            const response = await fetch(
                `${APP_CONFIG.apiBaseUrl}/api/v1/auth/oauth/${oAuthResult.provider}`,
                {
                    method: "POST",
                    headers: {
    ...
    async performOAuthFlow(provider = "google"): Promise<void> {
        ...
        if (!oAuthResult.success) {
            this.sendToWebView(
                "OAUTH_CANCELLED",
                oAuthResult as unknown as Record<string, unknown>,
            );
            return;
        }
        ...
        `);
            ...
            this.sendToWebView(
                "OAUTH_ERROR",
                backendResult as unknown as Record<string, unknown>,
            );
        }
    }
