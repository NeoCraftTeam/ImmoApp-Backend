...
class NativeService {
    ...
    async handleWebViewMessage(event: {
        ...
        try {
            switch (type) {
                ...
                default:
                    if (__DEV__)
                        console.log("[NativeService] Unhandled:", type);
            }
        } catch (err) {
    ...
    private triggerHaptic(style: HapticStyle): void {
        ...
        if (NOTIFICATION_HAPTICS.includes(style)) {
            Haptics.notificationAsync(
                mapped as Haptics.NotificationFeedbackType,
            );
        } else {
    ...
    async pickImage(options: Record<string, unknown> = {}): Promise<void> {
        try {
            ...
            this.sendToWebView("IMAGE_ERROR", {
    ...
    async takePhoto(options: Record<string, unknown> = {}): Promise<void> {
        try {
            ...
            this.sendToWebView("CAMERA_ERROR", {
                error: err instanceof Error ? err.message : "Unknown error",
            });
        }
    }
        ...
        try {
            ...
            if (status !== "granted") {
                this.sendToWebView("LOCATION_PERMISSION_DENIED", {
                    message: "Permission refusée pour la localisation",
                });
                return;
            }
            ...
            const timeoutPromise = new Promise<never>((_, reject) =>
                setTimeout(
                    () => reject(new Error("Localisation timeout")),
                    15_000,
                ),
            );
            ...
            this.sendToWebView("LOCATION_ERROR", {
                error: err instanceof Error ? err.message : "Unknown error",
            });
        }
    }
    ...
    private setupNotificationListeners(): void {
        ...
        this.responseListener =
            Notifications.addNotificationResponseReceivedListener((r) => {
                const data = r.notification.request.content.data as Record<
                    string,
                    unknown
                >;
    ...
    private async setBiometric(enabled: boolean): Promise<void> {
        if (enabled) {
            ...
            if (!passed) {
                this.sendToWebView("BIOMETRIC_SET_RESULT", {
                    success: false,
                    enabled: false,
