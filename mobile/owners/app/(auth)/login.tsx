...
/**
 * Owner login — progressive email → password disclosure, then a POST to
 * `/auth/login` → dashboard. Only the email field + CTA render first; the
 * password field animates in (height + opacity + slight slide) once the
 * email looks plausible, on blur, or on a first "Se connecter" tap. On
 * success a brief check-pop overlay plays before pushing into the
 * dashboard. All motion is gated by `useReducedMotion`.
 */
export default function Login() {
  ...
  const playSuccess = (onComplete: () => void) => {
    ...
    // Reduced motion : fade seul (pas de pop scale sur le check).
    if (reducedMotion) {
