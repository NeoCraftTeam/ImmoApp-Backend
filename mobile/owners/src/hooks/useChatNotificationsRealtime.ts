  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribePrivate(
      ...
      (_event, raw) => {
        ...
        const cached = qc.getQueryData<ConvListResponse>(['owner-conversations']);
        const exists = cached?.data?.some((c) => c.uuid === convUuid) ?? false;
        if (exists) {
          qc.setQueryData<ConvListResponse>(['owner-conversations'], (prev) => {
            ...
            const updated = prev.data.map((c) =>
              c.uuid === convUuid
                ? {
                    ...
                    unread_count: threadOpen ? c.unread_count : c.unread_count + 1,
                  }
                : c,
            );
            updated.sort((a, b) =>
        ...
        qc.invalidateQueries({ queryKey: ['owner-conversation-meta', convUuid] });
