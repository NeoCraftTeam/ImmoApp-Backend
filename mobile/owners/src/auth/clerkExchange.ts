...
/**
 * Swap a Clerk session JWT for a Sanctum bearer token — **owner / bailleur** flow.
 * `login_context: 'owner'` + `registration_intent: 'agent'` so the backend creates
 * (or connects) an AGENT account. OAuth (Google/Facebook/GitHub) : compte créé
 * immédiatement, sans OTP Laravel (l'email est déjà vérifié par le provider).
 */
