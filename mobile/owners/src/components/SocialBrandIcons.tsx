...
/**
 * Vraies marques Google (multicolore) / Facebook / GitHub en SVG, comme
 * l'app visitors — remplace les glyphes texte « G »/« f » peu fidèles.
 * GitHub prend la couleur de premier plan du thème (contraste clair/sombre).
 */
...
export function GoogleBrandIcon({ size = 20 }: { size?: number }): ReactElement {
  return <SvgXml xml={GOOGLE_SVG} width={size} height={size} />;
}

export function FacebookBrandIcon({ size = 20 }: { size?: number }): ReactElement {
...
export function GithubBrandIcon({
  ...
  return <SvgXml xml={githubSvg(color)} width={size} height={size} />;
}
