 ...
 * Owner variant — bundle `com.keyhome.owners`, scheme `keyhomeowners://auth/callback`.
 */
...
export function formatClerkAuthError(
  ...
  const redirectMismatch = raw.match(
    /authorized redirect URI[^.]*\.?\s*(exp:\/\/[^\s]+|keyhomeowners:\/\/[^\s]+)/i,
  );
  if (/redirect url passed in the sign in/i.test(raw) || redirectMismatch) {
    const url =
      redirectMismatch?.[1]
      ?? raw.match(/(exp:\/\/[^\s]+|keyhomeowners:\/\/[^\s]+)/i)?.[1]
      ?? 'keyhomeowners://auth/callback';
  ...
  if (/native api is disabled/i.test(raw)) {
    return [
      ...
      '2. Enregistrer l’app iOS (bundle : com.keyhome.owners)',
      '3. Ajouter l’URL de retour : keyhomeowners://auth/callback',
    ].join('\n');
  }
