...
`BoostPack`, `City`, `Dispute`, `DisputeEvidence`, `DisputeMessage`, `Document`, `EmailPreference`, `Expense`, `Invoice`, `LeaseContract`, `LeaseSignatureRequest`,
...
- `PaymentGatewayInterface` — payment gateway abstraction. Primary: `KpayPaymentService` (mobile money / Orange, Cameroun, pawaPay-backed hosted checkout). Also `StripePaymentService` (carte) and `FlutterwavePaymentService` (legacy rows/webhooks). Routed per-method via `PaymentMethod::gateway()` and the registry in `AppServiceProvider`.
...
- `DisputeService` — landlord/tenant dispute orchestrator (Centre de Résolution, modelled on PayPal/Airbnb).
  - `open(User $initiator, array $payload)` — **context-driven**: `respondent_id` is optional; if absent, `resolveRespondentFromContext()` derives it from `ad_id` (→ `Ad::user_id`), `lease_id` (→ other party on the lease), or `payment_id` (→ `Payment::user_id`, with ad-owner fallback). Self-dispute blocked at service level (422). Generates `KH-LITIGE-YYYY-XXXXXX` reference, sets SLA `+7 days`.
  - `addMessage()` — coerces `is_internal` to false for non-admins.
  - `uploadEvidence()` — stores files under `disputes/{id}/` on default disk.
  - `transition()` — validates against `DisputeStatus::canTransitionTo()`, sets `resolved_at` + `admin_id` on terminal states. All transitions notify both parties via `DisputeStatusChangedNotification`; `open()` also notifies admins.
  - **State machine** (`DisputeStatus` enum): `open → under_review → mediation → resolved_initiator | resolved_respondent | resolved_amicably`; `rejected` reachable from `open` or `under_review`.
  - **Backend**: Routes `/api/v1/disputes` (auth:sanctum, 10/h throttle on POST); admin-only `PATCH /disputes/{id}/status`. `StoreDisputeRequest` — `respondent_id` optional with `required_without_all:ad_id,lease_id,payment_id`. `DisputePolicy` is IDOR-proof (403 for non-party); blocks reply/upload on closed disputes. Filament admin resource `Litiges` (badge = open count). Permission `AdminPermission::DisputesManage`.
  - **Frontend** (`keyhome-frontend-next/`):
    - `components/disputes/OpenDisputeDialog.tsx` — multi-step wizard (context → pick → form). Pre-seedable via `initialContext` prop to skip steps 1+2. Ad picker shows only **unlocked ads** (`GET /my/unlocked-ads`) with client-side filter. Form validates inline (fields go red with char countdown before hitting API). Submit always enabled; validation errors shown via `setError` Alert on click.
    - `components/disputes/ReportAdButton.tsx` — "Signaler un problème" button; pre-seeds `OpenDisputeDialog` with `initialContext={{ kind: 'ad', ad }}`. Injected on `AdDetailClient.tsx` (mobile + desktop sidebar) gated on `isAuthenticated && !isOwnAd`.
    - `app/(dashboard)/litiges/page.tsx` — disputes list with status filter; opens `OpenDisputeDialog`.
    - `app/(dashboard)/litiges/[id]/page.tsx` — dispute detail: status stepper, messaging, evidence upload, admin transition panel (allowed_transitions), SLA deadline, resolution note.
    - `services/disputes.service.ts` — `list`, `get`, `create`, `sendMessage`, `uploadEvidence`, `transition`.
    - `types/dispute.ts` — full type mirror of `DisputeResource`; `CreateDisputePayload` with optional `respondent_id`.
  - **Tests**: `tests/Feature/DisputeFeatureTest.php` — 19 tests / 50 assertions covering creation, context-derivation, self-dispute rejection, IDOR, messaging, evidence, admin transitions, state machine, list scoping, reference pattern.
...
### Payment System
- **Gateways live: Kpay + Stripe (+ Flutterwave legacy).** `PaymentGateway` enum: `Kpay = 'kpay'`, `Flutterwave = 'flutterwave'` (legacy rows/webhook only), `Stripe = 'stripe'`. Stripe (carte, EUR via Cashier) unchanged since May 2026.
- **Strategy pattern + registry.** `PaymentGatewayInterface` implemented by `KpayPaymentService` (primary mobile money), `StripePaymentService`, and `FlutterwavePaymentService` (legacy). `AppServiceProvider` registers `kpay` as default + registry index `kpay|stripe|flutterwave`.
- **Routing.** `PaymentMethod::gateway()` : `MOBILE_MONEY` + `ORANGE_MONEY` + legacy `FLUTTERWAVE` method → **Kpay** ; `CARD` → Stripe.
...
- **Kpay API** (pawaPay-backed hosted GATEWAY checkout, docs `https://kpay.site/documentation`) : base `https://admin.kpay.site`, auth headers `X-API-Key` / `X-Secret-Key`, `POST /api/v1/payments/init` → `gatewayUrl` (customer picks their own Mobile Money operator on Kpay's page — we send only `amount`, `externalId` = KH `tx_ref`, `returnUrl`, `cancelUrl`, `metadata`), `GET /api/v1/payments/{id}` pour verify. `transaction_id` KeyHome reste `KH-*` ; l'`id` Kpay (`pay_*`) est stocké dans `gateway_response.kpay_id` et sert de référence verify. Webhook `POST /api/v1/webhooks/kpay` : HMAC-SHA256 sur le **raw body**, header `X-KPAY-Signature` (+ `X-KPAY-Event`) ; événements `payment.completed` / `payment.failed` (payload **plat** : `externalId`, `paymentId`, `status: COMPLETED`). Pas d'API refund marchande → `refund()` lève 501. **Webhook différé** : à activer plus tard côté dashboard Kpay ; `KPAY_WEBHOOK_SECRET` vide → les webhooks sont rejetés tant que le secret n'est pas configuré.
- **Webhooks.** Kpay: `POST /api/v1/webhooks/kpay`. Flutterwave (legacy): `POST /api/v1/webhooks/flutterwave`. Stripe: dedicated `POST /api/v1/webhooks/stripe` — Stripe requires the **raw body** for signature verification, controller passes `getContent()` and injects it as `__raw` so `StripePaymentService::handleWebhook()` can call `\Stripe\Webhook::constructEvent()`. `Cashier::ignoreRoutes()` is set in `AppServiceProvider::boot()` so Cashier's default `/stripe/webhook` does not collide.
...
- **Payment trace (`kh_payment_trace`).** Optional JSON blob under `gateway_response`, populated by Kpay, Flutterwave (legacy) and Stripe services with French-facing labels (PayPal, card brands, Apple/Google Pay, Orange/MTN hints). **`App\Support\PaymentPresentation`** is the single consumer for **`PaymentResource`** (`gateway_label`, `payment_method_label`, `payment_method_detail`), **`GET /payments/history`**, PDF `payment-history`, PWA tables, Filament Transactions/Refunds + Agency payments.
...
- **Crédits (`POST /credits/purchase/{package}`)** — accepte `callback_url` optionnelle ; validée par `FrontendRedirectGuard`. Passée à Kpay comme `returnUrl` / `cancelUrl` (config `KPAY_REDIRECT_URL` ou callback client).
- **Frontend** : `PaymentGateway` inclut `'kpay'` ; `usePayment` redirige vers hosted checkout pour tout gateway ≠ `stripe` (Kpay + legacy Flutterwave). Stripe : `<Elements>` + `clientSecret`.
- **Env backend** : `KPAY_API_KEY`, `KPAY_API_SECRET`, `KPAY_WEBHOOK_SECRET` (vide tant que les webhooks ne sont pas activés), optionnel `KPAY_BASE_URL` (déf. `https://admin.kpay.site`), `KPAY_REDIRECT_URL`, `PAYMENT_DEFAULT_GATEWAY=kpay`. Pas de clé publique frontend Kpay.
...
- **Parité auto-save** : `AdFormWizard.autoSaveData` ne sérialisait qu'un sous-ensemble des `AdFormValues` ; tout ce qui n'était pas envoyé restait uniquement en mémoire navigateur tant que l'utilisateur n'avait pas cliqué « Enregistrer le brouillon ». Désormais la snapshot inclut **tous** les champs supportés par `AdStatusController::autosave` : `charges_montant_forfait`, `charges_eau`, `charges_electricite`, `charges_autres`, et **`attributes`** (équipements). Médias (photos / panoramas / PDF) restent volontairement hors auto-save — communiqué via un helperText dédié sous le bouton.

  > **Note (Juin 2026)** : les cinq champs `distance_*_m` (route principale, commerces, transport, école, hôpital) ont été retirés du formulaire et de la table `ad`. La proximité est désormais calculée serveur via `NeighborhoodScorecardService` et exposée par `/api/v1/ads/{ad}/keyscore` + `/neighborhood-scorecard` ; le détail-annonce alimente ses chips depuis ces endpoints. Voir migration `2026_06_14_140813_drop_distance_fields_from_ads_table`.
