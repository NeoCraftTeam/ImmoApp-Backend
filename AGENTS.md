...
- **WebAuthn/Passkeys** (`laragear/webauthn ^5`):
  ...
  - **Frontend (Next.js) passkey integration**:
    ...
    - **`WebAuthnApiController` tokens**: issued via `LoginService::issueApiTokenForLoginContext()` → same `TokenService::rotateForUser()` naming as password login (`owner_token_*` / `client_token_*`) and the same Sanctum abilities (`role:<enum>`, `api:access`), so `AuthController::refresh()` and `EnsureTokenMatchesRole` behave consistently.
