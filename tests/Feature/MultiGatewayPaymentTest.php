...
it('uses primary gateway when it succeeds', function (): void {
    $primaryMock = makeGatewayMock(PaymentGateway::Kpay->value, [
        'link' => 'https://admin.kpay.site/gateway/MTX-001',
        'tx_ref' => 'KH-TEST001',
        'status' => 'pending',
        'gateway' => 'kpay',
    ]);

    $this->app->bind(KpayPaymentService::class, fn () => $primaryMock);

    config(['payment.default' => 'kpay', 'payment.fallback' => null]);
    ...
    expect($result['gateway'])->toBe('kpay')
        ->and($result['link'])->toBe('https://admin.kpay.site/gateway/MTX-001');
});

it('falls back to secondary gateway when primary fails', function (): void {
    $primaryMock = makeGatewayMock(
        PaymentGateway::Kpay->value,
        new PaymentGatewayException('Kpay: connexion echouee.')
    );
...
it('propagates primary exception when no fallback is configured', function (): void {
    $primaryMock = makeGatewayMock(
        PaymentGateway::Kpay->value,
        new PaymentGatewayException('Kpay: cle API invalide.')
    );
    ...
    expect(fn () => app(PaymentService::class)->createPayment($user, [
        ...
        'type' => 'credit',
        'payment_method' => 'mobile_money',
    ]))->toThrow(PaymentGatewayException::class, 'Kpay: cle API invalide.');
});
