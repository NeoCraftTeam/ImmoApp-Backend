...
it('reports turnstile as not required for native mobile app requests even when configured', function (string $client): void {
    Config::set('services.turnstile.site_key', 'test-turnstile-site-key');
    Config::set('services.turnstile.secret_key', 'sk-test-non-dummy-secret-value-xxxxxxxx');

    $this->withHeaders(['X-KeyHome-Client' => $client])
        ->getJson('/api/v1/config/turnstile')
        ->assertSuccessful()
        ->assertJsonPath('data.site_key', 'test-turnstile-site-key')
